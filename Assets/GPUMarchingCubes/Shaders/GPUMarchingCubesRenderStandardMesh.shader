	...
	SubShader
	{
		...
		Pass{
			...
			void geom_light(point v2g input[1], inout TriangleStream<g2f_light> outStream)
			{
				...
				for (i = 0; i < 5; i++) {
					...
					for (j = 0; j < 3; j++) {
						...
						o.worldPos = mul(unity_ObjectToWorld, float4(edgeVertices[vindex], 1));
						o.pos = UnityObjectToClipPos(o.worldPos);
						o.normal = norm;
						o.lightDir = normalize(ObjSpaceLightDir(float4(edgeVertices[vindex],1)));
						o.viewDir = normalize(ObjSpaceViewDir(float4(edgeVertices[vindex],1)));
			...
			fixed4 frag_light(g2f_light i) : SV_Target
			{
				...
				//float3 diffuse = diffStrength * _DiffuseColor.rgb * _LightColor0.rgb;
				//float3 specular = pow(specStrength, _SpecularPower) * _SpecularColor.rgb * _LightColor0.rgb;

				//fixed4 col;
				//col.rgb = (diffuse + specular) * atten + UNITY_LIGHTMODEL_AMBIENT.rgb * diffuse;
				//col.a = 1;
				//return col;
				...
				fixed4 color;
				//color.rgb = UNITY_LIGHTMODEL_AMBIENT.rgb * 2 * _DiffuseColor.rgb;
				...
				//color.rgb += lerp(_DiffuseColor.rgb * _LightColor0.rgb * diff, _LightColor0.rgb * _SpecularColor.rgb * spec, diff * spec) * (atten * 2);
